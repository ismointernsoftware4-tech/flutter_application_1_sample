...
class BranchFormProvider extends ChangeNotifier {
  BranchFormProvider({String? clinicId}) : _clinicId = clinicId;

  final FirebaseService _firebaseService = FirebaseService();
  static const String _formId = 'branch_form';
  ...
  Future<void> loadDefinition() async {
    ...
    try {
      ...
      debugPrint('Branch form load failed: $e');
      _definition ??= await _loadFromAsset();
    } finally {
      _isLoading = false;
      notifyListeners();
  ...
  Future<DynamicFormDefinition> _loadFromAsset() async {
    ...
    debugPrint('Branch form not found in Firebase. Using empty form definition.');
    return DynamicFormDefinition(
      id: _formId,
      name: 'Branch Registration Form',
      sections: [],
    );
  }
}
